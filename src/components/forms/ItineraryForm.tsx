...
import { Itinerary, UserRole, ItineraryCollateral, CollateralType, User } from '@/types';
...
const ItineraryForm: React.FC<ItineraryFormProps> = ({ onClose, onSubmit, itineraryToEdit }) => {
  ...
  const { users } = useData();
  const agents = useMemo(() => users.filter((u: User) => u.roles.includes(UserRole.AGENT)), [users]);
  ...
  const handleCollateralChange = (index: number, field: keyof ItineraryCollateral, value: string | CollateralType) => {
    ...
    const collateralToUpdate = { ...newCollaterals[index], [field]: value };
    newCollaterals[index] = collateralToUpdate;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (itineraryToEdit) {
        onSubmit({ ...itineraryToEdit, ...itineraryData });
    } else {
