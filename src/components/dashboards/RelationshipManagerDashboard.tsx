...
const RelationshipManagerDashboard: React.FC = () => {
  ...
  const assignedCustomers = customers.filter((c: Customer) => c.assignedRmId === user?.id);
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {assignedCustomers.map((customer: Customer) => {
                const agent = users.find((u: User) => u.id === customer.registeredByAgentId);
                return (
                    <tr key={customer.id}>
