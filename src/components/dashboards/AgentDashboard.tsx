...
import { Booking, Itinerary, Customer } from '@/types';
...
const AgentDashboard: React.FC = () => {
  ...
  const agentItineraries = useMemo(() => {
    return itineraries.filter((it: Itinerary) => it.assignedAgentId === user?.id || !it.assignedAgentId)
  }, [user, itineraries]);
  ...
  const agentItinerariesWithBookings: (Itinerary & { bookingCounts: { Pending: number; Confirmed: number; Completed: number; }})[] = useMemo(() => {
    if (!user) return [];

    const agentCustomers = customers.filter((c: Customer) => c.registeredByAgentId === user.id);
    const agentCustomerIds = new Set(agentCustomers.map((c: Customer) => c.id));
    const agentBookings = bookings.filter((b: Booking) => agentCustomerIds.has(b.customerId));

    return agentItineraries.map((itinerary: Itinerary) => {
      const bookingsForItinerary = agentBookings.filter((b: Booking) => b.itineraryId === itinerary.id);
      const bookingCounts = {
        Pending: bookingsForItinerary.filter((b: Booking) => b.status === 'Pending').length,
        Confirmed: bookingsForItinerary.filter((b: Booking) => b.status === 'Confirmed').length,
        Completed: bookingsForItinerary.filter((b: Booking) => b.status === 'Completed').length,
      };
      return {
        ...itinerary,
        bookingCounts,
