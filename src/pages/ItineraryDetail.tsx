...
import { CollateralType, UserRole, Itinerary, User, ItineraryCollateral } from '@/types';
import DashboardLayout from '@/components/shared/DashboardLayout';
import Card from '@/components/shared/Card';
import Button from '@/components/shared/Button';
import { DownloadIcon, PdfFileIcon, VideoIcon, GenericFileIcon } from '@/components/shared/icons/Icons';

const ItineraryDetail: React.FC = () => {
  const params = useParams<{ id: string }>();
  const { user } = useAuth();
  const { itineraries, users } = useData();
  const itinerary = itineraries.find((it: Itinerary) => it.id === params.id);
  const agent = users.find((user: User) => user.id === itinerary?.assignedAgentId);
  
  const isCustomer = user?.roles.includes(UserRole.CUSTOMER);

  // Filter collaterals: show all for non-customers, only approved for customers
  const visibleCollaterals = isCustomer 
    ? itinerary?.collaterals?.filter((c: ItineraryCollateral) => c.approved) ?? []
  ...
  const handleDownload = (collateral: ItineraryCollateral) => {
    // In a real app, this would trigger a download from a URL. Here, we mock it.
            ...
            <Card>
              ...
              {visibleCollaterals.length > 0 ? (
                <ul className="space-y-3">
                  {visibleCollaterals.map((collateral: ItineraryCollateral) => (
                    <li key={collateral.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      <div className="flex items-center space-x-3 flex-shrink-0">
                        ...
                        <button
                           ...
                           disabled={!collateral.approved && isCustomer}
                           className={`p-2 rounded-md transition-colors ${
                            (collateral.approved || !isCustomer) 
                            ? 'bg-blue-100 text-primary hover:bg-blue-200' 
                            : 'bg-gray-200 text-gray-400 cursor-not-allowed'
                           }`}
                           title={ (collateral.approved || !isCustomer) ? `Download ${collateral.name}` : 'Approval pending'}
                        >
